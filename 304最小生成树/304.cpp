...
int main(int argc, const char * argv[]) {
    ...
    for (i = 0; i < m; i++) {//对所有的边进行遍历
        ...
        for(i = 0; i < n; i++){//将新加入的顶点的标记全部更改为一样
            if(assist[i].camp == assist[end].camp){//如果有顶点标记与新加入边尾的标记相同
                assist[i].camp = assist[initial].camp;// 将该顶点标记改为边的第一个顶点的标记
            }
        }
        if (num == n-1) {//如果选择的边的数量和顶点数相差1，说明最小生成树已经形成，退出循环
            break;
        }
    }
    //输出
