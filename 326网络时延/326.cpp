...
int main(int argc, const char * argv[]) {
    ...
    wesum[k] = 0;// 存储节点权值，初始化为0
    while (!q.empty()) {// 当队列不为空时
        int t = q.front();// 定义t为队头，第一个就是k节点
        q.pop();// 弹队列
        flag[k] = 1;// 弹出，表示已经访问过该节点
        for (list<Edge>::iterator it = E[t].begin(); it != E[t].end(); it++) {// 使用迭代器，遍历链表
            if(wesum[it -> v] > wesum[t] + it -> w){// 找到距离k最近的节点作为跳板
                wesum[it -> v] = wesum[t] + it -> w;// 更新路径weight
